#include "arduinoshow.h"            // Benötigte Header-Datei einbinden
...
void ArduinoShow::on_listButton_clicked()  {             // Wenn man auf "Anschluss" klickt, werden die
                                                        //  verfügbaren USB-Ports angezeigt
...
{                                                   // zur Verbindung mit dem Arduino vorbereitet
    ...
    serialPort->setPortName(ui->portNameEdit->toPlainText());
                                                            // Einstellen der Verbdindungseigenschaften durch den Adruino gegeben
    serialPort->setBaudRate(QSerialPort::Baud9600);         //  Symbolrate
    serialPort->setDataBits(QSerialPort::Data8);            // Daten pro Frame
    serialPort->setParity(QSerialPort::NoParity);           // Überprüfungsbit
    serialPort->setStopBits(QSerialPort::OneStop);          // 1 Stopbit
    serialPort->setFlowControl(QSerialPort::NoFlowControl); // keine Reihenfolge kontrollieren

    if(serialPort->open(QIODevice::ReadWrite))              // Verbindung in beide Richtung nutzbar
    {
...
void ArduinoShow::on_initButton_clicked()  // Wenn man auf "Start" klickt passiert folgendes:
{
...
void ArduinoShow::handleErrors(QSerialPort::SerialPortError error)      // Anzeige bei Fehlern
{
    ...
    {
        serialPort->close();                                        // Verbindung unterbrechen
